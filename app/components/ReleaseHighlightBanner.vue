...
const rootClass = computed(() =>
  props.placement === 'header'
    ? 'flex min-w-0 max-w-[9.5rem] flex-col items-end justify-center xl:max-w-[11rem]'
...
const lineClass = computed(() =>
  props.placement === 'header'
    ? 'inline-flex max-w-full flex-wrap items-center justify-end gap-x-1 gap-y-0.5 text-[10px] leading-tight text-slate-400 xl:text-[11px]'
...
const closeClass = computed(() =>
  props.placement === 'header'
    ? '-mr-0.5 inline-flex size-6 shrink-0 items-center justify-center rounded-md text-slate-500 transition-colors hover:bg-white/8 hover:text-slate-300 focus:outline-none focus-visible:ring-1 focus-visible:ring-white/15'
