...
const posterSrc = computed(() => {
  const u = (props.card.posterUrl ?? '').trim()
...
<template>
  <div
    ...
    @click="handleClick"
  >
    <!-- Poster — AppImage: retry + no-referrer (fewer broken thumbnails) -->
    <AppImage
      :src="posterSrc"
      :alt="displayTitle"
      aspect-ratio="2/3"
    ...
    >
      ...
      <span
        v-if="kindLabel"
        class="pointer-events-none absolute left-1.5 top-1.5 rounded-md bg-black/60 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white backdrop-blur-sm"
      >
        {{ kindLabel }}
      </span>
    </AppImage>
